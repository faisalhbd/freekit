...
wait_for_service() {
	...
	while [ "$attempt" -le "$max_attempts" ]; do
		if curl -s --connect-timeout 2 --max-time 30 "http://$host:$port" >/dev/null 2>&1; then
	...
	echo "WARNING: $service_name health check timed out, but server may still be starting..."
	return 0
}
...
echo "[BUN] Starting development server (webpack mode for memory efficiency)..."
# Use webpack instead of Turbopack to reduce memory usage with 115+ routes
# Dynamic imports handle heavy libraries (tesseract.js, pdfjs-dist, pdf-lib)
NODE_OPTIONS='--max-old-space-size=2048' npx next dev -p 3000 --hostname 0.0.0.0 --webpack > "$PROJECT_DIR/dev.log" 2>&1 &
DEV_PID=$!
