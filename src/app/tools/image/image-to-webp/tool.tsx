...
import { Upload, Download, X, ImagePlus, RotateCcw, Loader2, Info, FileImage, Zap, ArrowRight } from "lucide-react"
...
function getOriginalFormat(file: File): string {
  const type = file.type.split("/")[1]?.toUpperCase() || "Unknown"
  ...
  const convertAll = useCallback(async () => {
    ...
    for (const image of currentImages) {
      ...
      try {
        ...
        const result = await convertToWebP(img, quality)
        setImages((prev) =>
          prev.map((i) =>
            i.id === image.id
              ? {
      ...
      <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-center rounded-full bg-primary/10 p-3 transition-colors group-hover:bg-primary/15">
            <FileImage className="size-6 text-primary" aria-hidden="true" />
          </div>
            ...
            {images.length === 0 ? (
              <>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  PNG, JPG, GIF, BMP, SVG — up to 50MB each
                </p>
              </>
            ) : (
              <>
        ...
        {/* 2. Quality Settings — Show after upload */}
        {images.length > 0 && (
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm">
              ...
              {doneCount > 0 && totalConvertedSize > 0 && (
                <span className="font-medium text-emerald-600 dark:text-emerald-400">
                  WebP: {formatFileSize(totalConvertedSize)}
              ...
              {doneCount > 0 && (
                <Tooltip>
                  ...
                  <TooltipContent>Download all WebP images</TooltipContent>
                </Tooltip>
              )}
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="relative aspect-video overflow-hidden rounded-lg bg-muted">
                      ...
                      <span className="absolute bottom-1 left-1 rounded bg-background/80 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                        {getOriginalFormat(image.file)} {formatFileSize(image.originalSize)}
                      </span>
                    </div>
                    <div className="relative aspect-video overflow-hidden rounded-lg bg-muted">
                      {image.status === "done" && image.convertedUrl ? (
                        ...
                        <div className="flex size-full flex-col items-center justify-center gap-1">
                          ...
                          <span className="text-xs text-muted-foreground">WebP</span>
                        </div>
                      )}
                    </div>
                  </div>
